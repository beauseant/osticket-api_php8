...
class Ticket 
{
        function compileResults($result)
        {
            return array(
                    ...
                    'flags'=>$result->flags,
                    'sort'=>$result->sort,
                    'subject'=>mb_convert_encoding($result->subject,'UTF-8'),
                    'title'=>mb_convert_encoding($result->title,'UTF-8'),
        ...
        public function add($parameters)
        {
            ...
            Helper::checkRequest($parameters, $expectedParameters);
                ...
                $ticket__cdata .= '"'. mb_convert_encoding($parameters["parameters"]["subject"],'UTF-8').'",';
                ...
                $thread_entry .= '65,';
                $thread_entry .= '"API",';
                $thread_entry .= '"'. mb_convert_encoding($parameters["parameters"]["title"],'UTF-8').'",';
                $thread_entry .= '"<p>'. mb_convert_encoding($parameters["parameters"]["subject"],'UTF-8').'</p>",';
                $thread_entry .= 'now(),';    
                $thread_entry .= 'now())';
        ...
        public function reply($parameters)
        {
            ...
            Helper::checkRequest($parameters, $expectedParameters);
                ...
                $thread_id = $row->id;
                    ...
                    $thread .= '"<p>'. mb_convert_encoding($parameters["parameters"]["body"],'UTF-8').'</p>",';
                    $thread .= '"API",';  
                    $thread .= '"R",';                                       
                    $thread .= 'now(),';    
                    $thread .= 'now())';
